...
function Profile({doctorData,refreshData}) {
    ...
    //to handle form photo ops from cloudinary
    ...
    //submit for Update form(====Update function====)
    ...
    //for adding new qualification deleting and handling
    const addQualification=(e)=>{
        e.preventDefault();
        addItem('qualifications',
        {startingDate: '', endingDate: '', degree: 'PHD', university: 'AIIMS BBSR'}
        )
    }
   ...
   //for adding new Experience deleting and handling
   const addExperience=(e)=>{
        e.preventDefault();
        addItem('experiences',{
            startingDate: '', endingDate: '', position: 'Senior Surgeon', hospital: 'AIIMS BBSR'
        })
    }
   ...
   const addTimeSlot=(e)=>{
    ...
    return (
        <div>
            ...
            <form>
                <div className="mb-5">
                    <p className="form_label pb-2">Name*</p>
                    <input type="text" name='name' value={formData.name} onChange={handleInputChange} placeholder='Full Name' className='form__input' />
                </div>
                <div className="mb-5">
                    <p className="form_label pb-2">Email*</p>
                    <input type="email" name='email' value={formData.email} onChange={handleInputChange} placeholder='Email' className='form__input bg-indigo-100' readOnly aria-readonly disabled={true} />
                </div>
                <div className="mb-5">
                    <p className="form_label pb-2">Phone*</p>
                    <input type="number" name='phone' value={formData.phone} onChange={handleInputChange} placeholder='Phone' className='form__input' />
                </div>
                <div className="mb-5">
                    <p className="form_label pb-2">Bio*</p>
                    ...
                    {formData.qualifications?.map((item, index) => (
                        <div key={index}>
                            <div>
                                ...
                                <button onClick={e=>deleteQualification(e,index)} className='bg-red-600 p-2 rounded-full text-white text-[18px] mb-[30px] mt-[5px] cursor-pointer'>
                                    <AiOutlineDelete />
                                </button>
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-5">
                                    <div>
                                        ...
                                        <input type="text" name='position' value={item.position} className='form__input' placeholder='Position' onChange={e=>handleExperienceChange(e,index)}/>
                                    </div>
                                    <div>
                                        <p className="form__label">Hospital*</p>
                                        <input type="text" name='hospital' value={item.hospital} className='form__input' placeholder='Hospital' onChange={e=>handleExperienceChange(e,index)}/>
                                    </div>
                                </div>
                                <button onClick={e=>deleteExperience(e,index)} className='bg-red-600 p-2 rounded-full text-white text-[18px] mb-[30px] mt-[5px] cursor-pointer'>
                                    <AiOutlineDelete />
                                </button>
                            </div>
                        </div>
                            ...
                            <div>
                                <div className="grid grid-cols-2 md:grid-cols-4 mb-[30px] gap-5">
                                    ...
                                    <div className='flex items-center'>
                                        <button onClick={e=>deleteTimeSlot(e,index)} className='bg-red-600 p-2 rounded-full text-white text-[18px] mb-[30px] mt-12 cursor-pointer'>
                                            <AiOutlineDelete />
                                        </button>
                                    </div>
                                </div>
                ...
                <div className="mb-5 flex items-center gap-3">
                    {formData.photo && <figure className="w-[60px] h-[60px] rounded-full border-2 border-solid border-primaryColorflex items-center justify-center">
